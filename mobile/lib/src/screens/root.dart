    ...
    return ListenableBuilder(
      ...
      builder: (context, _) {
        ...
        if (session.identityError != null) {
          ...
          return Scaffold(
            appBar: AppBar(
              title: Text(offline ? tr('لا يوجد اتصال') : tr('تعذّرت قراءة حسابك')),
...
String identityHint(String? code, [Duration? drift]) => switch (code) {
  ...
  jwtIssuedAtFuture =>
    trf(
        ...
        // الرقم المقيس إن أمكن قياسه: «سبع دقائق» تُفعَل، و«فرقٌ في
        // الساعة» لا تُفعَل.
        [clockSkewLabel(drift) == null ? '' : '${clockSkewLabel(drift)}\n\n']),
  // جدولٌ أو طريقةٌ غير موجودة.
  '42P01' || 'PGRST205' =>
    tr('الغالب أن ملفات المخطّط في مجلّد supabase/ لم تُطبَّق على المشروع بعد — '
        'المصادقة تعمل بلا جداولنا، فالدخول ينجح وحده ثم يقف كل شيء.'),
  // عمودٌ غير موجود: مخطّطٌ أقدمُ من التطبيق.
  '42703' =>
    tr('قاعدتك أقدمُ من هذه النسخة من التطبيق: عمودٌ يطلبه غيرُ موجود. شغّل '
        'ملفات supabase/ الأحدث ثم أعد المحاولة.'),
  // صلاحية.
  '42501' || 'PGRST301' =>
    tr('القاعدة رفضت القراءة لسياسة صلاحيات. تأكّد من تطبيق policies.sql على '
        'المشروع.'),
  _ =>
    tr('إن كنت لم تُطبّق ملفات مجلّد supabase/ على المشروع بعد فابدأ بها — '
        'المصادقة تعمل بلا جداولنا، فالدخول ينجح وحده ثم يقف كل شيء.'),
};
