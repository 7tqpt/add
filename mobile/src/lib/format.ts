/**
 * نسخة من `src/lib/format.ts` في اللوحة.
 *
 * التكرار هنا لا يُتجنَّب بسهولة: مشروع Metro لا يستورد من خارج جذره بلا إعداد
 * إضافي، وربط المشروعين بمساحة عمل واحدة ثمنه تعقيد في البناء السحابي. والملف
 * خالص من أي اعتماد — دوال نصّية فقط — فنسخه أرخص من ربطه.
 *
 * Arabic locale with Latin digits and the Gregorian calendar: Arabic separators
 * and month names, but numerals that stay legible next to version strings and
 * chart axes.
 */
const LOCALE = 'ar-EG-u-nu-latn-ca-gregory'

const int = new Intl.NumberFormat(LOCALE, { maximumFractionDigits: 0 })
const oneDecimal = new Intl.NumberFormat(LOCALE, {
  minimumFractionDigits: 1,
  maximumFractionDigits: 1,
})
const percent = new Intl.NumberFormat(LOCALE, {
  style: 'percent',
  maximumFractionDigits: 1,
})
const dayMonth = new Intl.DateTimeFormat(LOCALE, { day: 'numeric', month: 'short' })
const fullDate = new Intl.DateTimeFormat(LOCALE, {
  day: 'numeric',
  month: 'short',
  year: 'numeric',
})
const dateTime = new Intl.DateTimeFormat(LOCALE, {
  day: 'numeric',
  month: 'short',
  hour: 'numeric',
  minute: '2-digit',
})

export const formatNumber = (n: number) => int.format(n)

/** The four Arabic forms a counted noun takes: مفرد، مثنّى، جمع قلّة، تمييز مفرد. */
export interface ArabicNoun {
  one: string
  two: string
  few: string
  many: string
}

export const DAY_FORMS: ArabicNoun = { one: 'يوم', two: 'يومين', few: 'أيام', many: 'يوماً' }
export const HOUR_FORMS: ArabicNoun = { one: 'ساعة', two: 'ساعتين', few: 'ساعات', many: 'ساعة' }
export const MINUTE_FORMS: ArabicNoun = { one: 'دقيقة', two: 'دقيقتين', few: 'دقائق', many: 'دقيقة' }
export const BOOKING_FORMS: ArabicNoun = {
  one: 'حجز واحد',
  two: 'حجزان',
  few: 'حجوزات',
  many: 'حجزاً',
}

/**
 * "3 أيام" / "يومين" / "12 يوماً".
 *
 * Arabic agreement is not a plural `s`: two takes its own dual form and drops
 * the numeral, 3–10 take the broken plural, and 11 and up take a singular
 * accusative. Printing "2 يوم" reads as broken text, not as a rounding choice.
 */
export function formatCount(count: number, forms: ArabicNoun): string {
  const n = Math.abs(count)
  if (n === 1) return forms.one
  if (n === 2) return forms.two
  if (n >= 3 && n <= 10) return `${int.format(n)} ${forms.few}`
  return `${int.format(n)} ${forms.many}`
}

/** 12_400 → "12.4 ألف". Used on axis ticks and tight stat tiles. */
export function formatCompact(n: number): string {
  const abs = Math.abs(n)
  if (abs >= 1_000_000) return `${oneDecimal.format(n / 1_000_000)} م`
  if (abs >= 10_000) return `${int.format(Math.round(n / 1000))} ألف`
  if (abs >= 1000) return `${oneDecimal.format(n / 1000)} ألف`
  return int.format(n)
}

export const formatPercent = (fraction: number) => percent.format(fraction)

/**
 * Yemeni Rial. The suffix is appended manually rather than via
 * `style: 'currency'`: the CLDR symbol carries a trailing period that bidi
 * reordering strands at the far edge of the number ("‎.145,873 ر.ي").
 */
export const formatMoney = (n: number) => `${int.format(n)} ر.ي`

/** For tight columns where the full figure would wrap: "1.2 م ر.ي". */
export const formatMoneyCompact = (n: number) => `${formatCompact(n)} ر.ي`

/** Signed percentage for delta cues: "+12.4%" / "−3.1%". */
export function formatDelta(fraction: number): string {
  const sign = fraction > 0 ? '+' : fraction < 0 ? '−' : ''
  return `${sign}${percent.format(Math.abs(fraction))}`
}

export const formatDayMonth = (iso: string) => dayMonth.format(new Date(iso))

export const formatDate = (iso: string) => fullDate.format(new Date(iso))

export const formatDateTime = (iso: string) => dateTime.format(new Date(iso))

/** "منذ 3 ساعات". Falls back to an absolute date past a month. */
export function formatRelative(iso: string | null): string {
  if (!iso) return '—'
  const diffMs = Date.now() - new Date(iso).getTime()
  // المستقبل يُصاغ بـ«بعد» لا بـ«منذ». وبلا هذا كان كل تاريخ قادم يُقرأ «الآن»
  // لأن الفرق سالب فيقع تحت الدقيقة — فبدت دعوةٌ تنتهي بعد أسبوع منتهيةً للتوّ.
  if (diffMs < -60_000) return formatUntil(-diffMs)
  const minutes = Math.round(diffMs / 60_000)
  if (minutes < 1) return 'الآن'
  if (minutes < 60) return `منذ ${formatCount(minutes, MINUTE_FORMS)}`
  const hours = Math.round(minutes / 60)
  if (hours < 24) return `منذ ${formatCount(hours, HOUR_FORMS)}`
  const days = Math.round(hours / 24)
  if (days < 30) return `منذ ${formatCount(days, DAY_FORMS)}`
  return fullDate.format(new Date(iso))
}

/** كم يبقى حتى لحظةٍ قادمة، بالمسافة الزمنية لا بالتاريخ. */
function formatUntil(ms: number): string {
  const minutes = Math.round(ms / 60_000)
  if (minutes < 60) return `بعد ${formatCount(minutes, MINUTE_FORMS)}`
  const hours = Math.round(minutes / 60)
  if (hours < 24) return `بعد ${formatCount(hours, HOUR_FORMS)}`
  const days = Math.round(hours / 24)
  if (days < 30) return `بعد ${formatCount(days, DAY_FORMS)}`
  return fullDate.format(new Date(Date.now() + ms))
}

/** 3_720 → "1 س 2 د". Session lengths, never wall-clock times. */
export function formatDuration(seconds: number): string {
  if (!Number.isFinite(seconds) || seconds <= 0) return '—'
  const hours = Math.floor(seconds / 3600)
  const minutes = Math.floor((seconds % 3600) / 60)
  // A whole number of hours reads as "4 س", not "4 س 0 د".
  if (hours > 0) return minutes === 0 ? `${int.format(hours)} س` : `${int.format(hours)} س ${int.format(minutes)} د`
  if (minutes > 0) return `${int.format(minutes)} د`
  return `${int.format(Math.round(seconds))} ث`
}

export const PLATFORM_LABEL: Record<string, string> = {
  ios: 'iOS',
  android: 'Android',
}

/** "20:00" → "8:00 م". Event times are stored as plain HH:MM. */
export function formatTime(value: string | null): string {
  if (!value) return '—'
  const [h, m] = value.split(':').map(Number)
  if (Number.isNaN(h)) return value
  const period = h < 12 ? 'ص' : 'م'
  const hour = h % 12 === 0 ? 12 : h % 12
  return `${hour}:${String(m ?? 0).padStart(2, '0')} ${period}`
}
