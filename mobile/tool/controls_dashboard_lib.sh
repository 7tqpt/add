...
# **ومرساتُه جُدّدت مرّتين، والثانيةُ كشفت شيئاً:**
#
#    الأولى: كان السطرُ في `escapeCell`، فلمّا خرج القرارُ إلى `neutralise`
#    لأجل حارس الصيغ لم تعد المرساةُ تُطابق شيئاً — وقال الضابطُ «لا كسرَ
#    وقع». **وهو صادقٌ في قوله: لا يُقاس بضابطٍ لم يكسر.**
#
#    والثانية: جُرّب **نزعُ السطر كلِّه** فبقيت الحزمةُ خضراء — لأنّ
#    `Array.prototype.join` يرسم `null` و`undefined` فراغاً من نفسِه
#    (‏`[null, undefined, 'x'].join(',') === ',,x'`‏). فالسلوكُ محروسٌ
#    بـ`join` لا بذلك السطر، ونزعُه لا يُغيّر حرفاً في الملفّ الخارج.
#
#    **فصار الكسرُ ما يُغيّر الخرجَ فعلاً**: تُعاد `String` على الفارغ فتخرج
#    كلمةُ «null» في كلّ خانةٍ لا قيمةَ لها في التقرير.
run "ك) الفارغُ يُكتب null" sub "$C" \
  "  if (value === null || value === undefined) return ''" \
  "  if (value === null || value === undefined) return String(value)"

# ل١) **وحارسُ صيغ Excel يسقط** — فاسمُ قاعةٍ يبدأ بـ`=` يُنفَّذ حين يفتح
#     صاحبُ المنصّة التقرير. وهو ثقبٌ لا عيبُ شكل.
run "ل١) لا حارسَ لصيغ Excel" sub "$C" \
  "  return FORMULA_START.test(value) ? \`'\${value}\` : value" \
  "  return value"

# ل٢) **ويُحرَس الرقمُ كما يُحرَس النصّ** — فـ‏`-500`‏ خلف شَرطةٍ يكفّ عن أن
#     يكون رقماً: لا يُجمع العمود، ويخرج تقريرُ المرتجعات بمجموعٍ صفرٍ بلا
#     رسالةِ خطأٍ واحدة. **وهذا أخطرُ من الثقب الذي يُسدّ.**
run "ل٢) الأرقامُ تُحرَس كالنصّ" sub "$C" \
