...
def trim_margin(im: Image.Image, fill: float = 0.5) -> Image.Image:
    """يقصّ الهامشَ حول العمل — **معتماً كان أو فاتحاً**.

    صور المولّدات تأتي بمربّعٍ مستدير الزوايا داخل خلفيةٍ سادة، وعليها أحياناً
    ...
    **والقاعدة أغلبيةٌ لا وجودٌ مفرد**، وهذا ما تعلّمتُه بتشغيلها: أوّل نسخةٍ
    ...
    العمل إذا كان أكثرُ نصفِه مخالفاً للهامش؛ والشارة تشغل جزءاً يسيراً من
    صفّها فتسقط معه.

    **والهامشُ يُقرأ من الزوايا لا يُفترض معتماً.** أوّلُ نسخةٍ سألت «أهو
    فاتحٌ؟» — وهي تعمل على خلفيةٍ سوداء وتعطب على بيضاء: أيقونةُ «فرحتي»
    الجديدةُ جاءت على أرضيةٍ بيضاء، فعُدّ الهامشُ كلُّه عملاً ولم يُقصّ شيءٌ
    ولا سقطت الشارة. فصار السؤالُ: **أيخالف هذا البكسلُ لونَ الزوايا؟**
    """
    ...
    # لونُ الهامش: متوسّطُ الزوايا الأربع. وأربعٌ لا واحدة — زاويةٌ قد تقع
    # عليها الشارةُ نفسُها.
    corners = [px[1, 1], px[w - 2, 1], px[1, h - 2], px[w - 2, h - 2]]
    edge = tuple(sum(c[i] for c in corners) / 4 for i in range(3))

    def lit(x: int, y: int) -> bool:
        r, g, b, a = px[x, y]
        if a <= 24:
            return False  # شفّافٌ: هامشٌ على كلّ حال
        return sum(abs(v - e) for v, e in zip((r, g, b), edge)) > 60
