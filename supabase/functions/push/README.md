...
| `PUSH_WEBHOOK_SECRET` | سرّ عشوائي طويل مخصّص للخطّاف؛ نفس القيمة تُحفظ في Vault باسم `push_webhook_secret` |
...
ولحفظ السرّ في **Supabase → Database → Vault** أضف سرّاً باسم
`push_webhook_secret` وقيمته هي نفسها `PUSH_WEBHOOK_SECRET`. لا تضع قيمته في
المستودع أو في نصّ المُشغّل. يستخدم `push_hook.sql` Vault مع `pg_net` لإرسال
السرّ في ترويسة الطلب وقت حدوث الإشعار.
...
**عند تحديث مشروع يعمل بالفعل:** اضبط السرّ في Edge وVault أولاً، ثم شغّل
`security_notification_rpcs.sql` لإغلاق مسارات SQL المكشوفة، ثم
`push_hook.sql` و`enable_push_webhook(...)` من هذه النسخة، وبعدها انشر دالّة
`push` الجديدة. إذا نُشرت الدالّة قبل تحديث المُشغّل، ستردّ طلباته القديمة
بـ`401` ولن تصل إشعارات الجوال حتى تُكمل خطوة SQL. أبقِ قيمة السرّ خارج سجلات
الأوامر والملفات المنشورة.
