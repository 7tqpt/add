...
# ── ح) وتحقُّقُ الملفِّ يعود يسأل عن الوجود لا عن الفعل ─────────────────────
#
# **وهذا ما وقع فعلاً:** نُفِّذ الملفُّ على الإنتاج فقال «٢ و١ و٤» — ولا
# سطرَ فيه يمسّ ما جاء الملفُّ ليُصلحه. فلو بقي `set null` في القاعدة
# لَقُرئ التقريرُ سليماً و«حذف حسابي» محبوسٌ كما كان.
run "(ح) التحقّقُ لا يسأل عن فعل المفتاح" \
  sub "$F" \
"   and con.confdeltype = 'c';" \
"   and true;"
