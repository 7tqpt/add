...
export function ProvidersPage() {
  ...
  useEffect(() => {
    setPage(0)
  }, [debouncedSearch, status, category, city])

  const load = useCallback(
    () =>
      listProviders({ search: debouncedSearch, status, category, city, page, pageSize: PAGE_SIZE }),
    [debouncedSearch, status, category, city, page],
  )
                ...
                {data.rows.map((provider) => (
                  ...
                  >
                    ...
                    <td className="tnum px-4 py-3 text-xs whitespace-nowrap text-ink-2">
                      {formatNumber(provider.completed_orders)}
                    </td>
                    <td className="tnum px-4 py-3 text-xs whitespace-nowrap text-ink-2">
